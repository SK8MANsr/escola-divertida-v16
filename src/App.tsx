...
const premiumSceneArt = {
  home: '/v10/home-premium.jpeg',
  rewards: '/v10/rewards-showcase.jpeg',
  parents: '/v10/parents-panel.jpeg',
  memory: '/v10/memory-premium.jpeg',
  map: '/v10/map-premium.jpeg',
};

const worldPosterArt: Record<GameKey, string> = {
  memory: premiumSceneArt.memory,
  alphabet: premiumSceneArt.home,
  math: premiumSceneArt.map,
  shape: premiumSceneArt.home,
  colors: premiumSceneArt.rewards,
  maze: premiumSceneArt.map,
  puzzle: premiumSceneArt.memory,
};

const symbolLabels: Record<string, string> = {
  '🐶': 'Cachorrinho', '🐱': 'Gatinho', '🦊': 'Raposinha', '🐸': 'Sapinho', '🐵': 'Macaco', '🐼': 'Panda', '🦁': 'Leão', '🐰': 'Coelho', '🐻': 'Urso', '🦉': 'Corujinha',
  '🍎': 'Maçã', '🍌': 'Banana', '🍇': 'Uvas', '🍉': 'Melancia', '🍊': 'Laranja', '🍓': 'Morango', '🥝': 'Kiwi', '🍒': 'Cereja', '🍍': 'Abacaxi', '🥥': 'Coco',
  '🐠': 'Peixinho', '🐙': 'Polvo', '🦀': 'Caranguejo', '🐳': 'Baleia', '🐬': 'Golfinho', '🦑': 'Lula', '🐡': 'Baiacu', '🪼': 'Água-viva', '🐚': 'Concha', '🐢': 'Tartaruga',
  '🌟': 'Estrela', '🌙': 'Lua', '🪐': 'Planeta', '🚀': 'Foguete', '☄️': 'Cometa', '👽': 'Alien', '🌍': 'Planeta Terra', '🛰️': 'Satélite', '⭐': 'Estrela dourada', '🌌': 'Galáxia',
  '🧸': 'Ursinho', '🎈': 'Balão', '🎲': 'Dado', '🎨': 'Tintas', '🐮': 'Vaquinha', '🐷': 'Porquinho', '🐔': 'Galinha', '🐑': 'Ovelha', '🐴': 'Cavalinho', '🐐': 'Cabra',
};

const symbolPalettes = [
  ['#7c3aed', '#d946ef', '#fde68a'],
  ['#0ea5e9', '#2563eb', '#fef3c7'],
  ['#22c55e', '#14b8a6', '#fef9c3'],
  ['#f97316', '#ef4444', '#fee2e2'],
  ['#ec4899', '#8b5cf6', '#fde68a'],
  ['#06b6d4', '#3b82f6', '#dbeafe'],
];

const paletteForSymbol = (symbol: string) => symbolPalettes[Math.abs(Array.from(symbol).reduce((a, c) => a + c.charCodeAt(0), 0)) % symbolPalettes.length];

const premiumMemoryArt = (symbol: string) => {
  const [a, b, c] = paletteForSymbol(symbol);
  const label = symbolLabels[symbol] ?? 'Tesouro';
  return `data:image/svg+xml;utf8,${encodeURIComponent(`
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 320">
      ...
      <rect x="14" y="14" width="292" height="292" rx="48" fill="url(#bg)"/>
      <rect x="28" y="28" width="264" height="264" rx="38" fill="${c}" opacity="0.22"/>
      <circle cx="88" cy="78" r="42" fill="white" opacity="0.2"/>
      <circle cx="238" cy="82" r="18" fill="white" opacity="0.15"/>
      <circle cx="244" cy="236" r="26" fill="white" opacity="0.12"/>
      <rect x="44" y="198" width="232" height="56" rx="28" fill="rgba(255,255,255,.2)"/>
      <text x="160" y="148" text-anchor="middle" font-size="100">${symbol}</text>
      <text x="160" y="233" text-anchor="middle" font-size="22" font-family="Arial, sans-serif" font-weight="700" fill="white">${label}</text>
      <path d="M64 62c14-22 31-30 48-32" stroke="white" stroke-width="8" stroke-linecap="round" opacity="0.35"/>
      <path d="M98 272c32 8 66 9 100 1" stroke="white" stroke-width="8" stroke-linecap="round" opacity="0.25"/>
    </svg>
  `)}`;
};

const premiumCardBack = `data:image/svg+xml;utf8,${encodeURIComponent(`
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 320">
    <defs>
      <linearGradient id="bg" x1="0" x2="1" y1="0" y2="1">
    ...
    <rect x="14" y="14" width="292" height="292" rx="48" fill="url(#bg)"/>
    <rect x="30" y="30" width="260" height="260" rx="38" fill="rgba(255,255,255,.12)"/>
    <circle cx="160" cy="160" r="78" fill="rgba(255,255,255,.14)"/>
    <path d="M160 95l18 38 42 6-31 30 7 42-36-19-36 19 7-42-31-30 42-6z" fill="#fde68a"/>
    <text x="160" y="262" text-anchor="middle" font-size="20" font-family="Arial, sans-serif" font-weight="700" fill="white">Escola Divertida</text>
  </svg>
`)}`;
  ...
  return (
    <div className="kid-shell">
      ...
      <div className="kid-main">
        ...
        <div className="quick-dock">{navItems.slice(0,6).map((item) => { const Icon = item.icon; return <button type="button" key={item.key} className={cn('quick-dock-item', panel === item.key && 'is-active')} onClick={() => setPanel(item.key)}><Icon size={18} /><span>{item.shortLabel}</span></button>; })}</div>
  ...
  return (
    <div className="window-grid two-columns">
      ...
      <div className="panel-column detail-column">
        {selectedPack ? (
          <>
            <div className="pack-hero-card premium-illustrated-card" style={{ background: `linear-gradient(135deg, ${selectedPack.colorA}, ${selectedPack.colorB})` }}>
            ...
            <div className="pack-poster-inline"><img src={premiumSceneArt.rewards} alt="Prévia premium de recompensas" /></div>
  ...
  return (
    <div className="event-grid">
      {events.map((event) => {
        ...
        return (
          <article key={event.id} className="event-card premium-event-card">
            ...
            <div className="event-art-strip"><img src={worldPosterArt[event.world]} alt={event.title} /></div><p>{event.subtitle}</p>
    ...
    <div className="window-grid two-columns">
      ...
      <div className="panel-column detail-column">
        <div className={cn('world-hero-card', worldAccent[world.game])}>
          <div className="world-hero-poster"><img src={worldPosterArt[world.game]} alt={world.title} /></div>
...
function PlayPanel({ packs, progressMap, selectedWorld, setSelectedWorld, onPlayPhase }: PlayPanelProps) {
  ...
  return (
    <div className="play-grid">
      <article className="play-launch-card is-primary premium-launch-card">
        <img src={worldPosterArt[world.game]} alt={world.title} className="launch-card-bg" />
      ...
      <article className="play-launch-card premium-launch-card">
        <img src={premiumSceneArt.rewards} alt="Modo surpresa" className="launch-card-bg" />
...
function RewardsPanel({ progressMap, totalStars, totalCompleted }: RewardsPanelProps) {
  return (
    <div className="rewards-layout premium-rewards-layout">
      <div className="reward-showcase-card">
        <img src={premiumSceneArt.rewards} alt="Painel premium de recompensas" className="reward-showcase-image" />
        <div className="reward-showcase-overlay">
          <strong>Cofre encantado</strong>
          <span>Recompensas ilustradas, baús mágicos e progresso visível.</span>
        </div>
      </div>
      ...
      <div className="reward-milestone-grid">
        {rewardMilestones.map((reward) => (
          <div key={reward.label} className={cn('reward-milestone-card', totalStars >= reward.threshold && 'is-unlocked')}>
            <span>{totalStars >= reward.threshold ? '🏆' : '🎁'}</span>
...
function ParentsPanel({ profile, tracks, infra }: ParentsPanelProps) {
  ...
  return (
    <div className="parents-grid premium-parents-grid">
      <article className="parent-dashboard-card parent-hero-image-card">
        <img src={premiumSceneArt.parents} alt="Painel premium dos pais" className="parent-showcase-image" />
      </article>
...
function GameStage({ phase, progress, soundEnabled, onPlayed, onComplete }: GameStageProps) {
  ...
  return (
    <div className="game-stage-shell">
      <div className="game-stage-summary premium-game-header">
        <div className="game-stage-poster"><img src={phase.game === 'memory' ? premiumSceneArt.memory : worldPosterArt[phase.game]} alt={phase.title} /></div>
...
function MemoryGame({ phase, onComplete }: { phase: MemoryPhase; onComplete: GameComplete }) {
  ...
  return (
    <div className="game-panel">
      <div className="game-helper">Encontre os pares iguais. Agora as cartas têm artes mais ricas, brilho premium e leitura fácil.</div>
      <div className="memory-grid" style={{ gridTemplateColumns: `repeat(${Math.min(4, Math.ceil(Math.sqrt(cards.length)))}, minmax(0, 1fr))` }}>
        {cards.map((card) => {
          ...
          return (
            ...
            >
              {flipped ? <img src={premiumMemoryArt(card.symbol)} alt={symbolLabels[card.symbol] ?? 'Carta'} className="memory-art" /> : <img src={premiumCardBack} alt="Verso da carta" className="memory-back-art" />}
            </button>
          );
        })}
      </div>
